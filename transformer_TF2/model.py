...
class Transformer_Encoder(tf.keras.layers.Layer):
    def __init__(self,
        ...
        self.encoder_embedding_layer = tf.keras.layers.Dense(d_model, activation=None)
    ...
    def call(self,
        ...
        # print('ffffffffffirsthear903254u813908456778902345')
        ...
        encoder_tensor = self.Pos_Enc(encoder_tensor)  ##position enc.
        # print("encoder embedding dropout:")
        encoder_tensor = self.encoder_embedding_dropout(encoder_tensor, training=training)
        # print("for encoder count:")
        for i in range(self.encoder_count):
            encoder_tensor, ATTN = self.encoder_layers[i](encoder_tensor, inputs_padding_mask, training=training)
            # print(ATTN)
            # print(tf.reshape(ATTN, (-1, )).numpy())
        # print("done model")
    ...
    def get_config(self):
        return {"inputs_vocab_size" : self.inputs_vocab_size,
...
class Transformer_Encoder_NoEMB(tf.keras.layers.Layer):
    def __init__(self,
                 inputs_vocab_size,
                 output_size,
                 encoder_count,
                 ...
                 d_point_wise_ff,
                 dropout_prob):
        super(Transformer_Encoder_NoEMB, self).__init__()
...
class Pos_Enc(tf.keras.layers.Layer):
    ...
    def positional_encoding(self, max_len):
        pos = np.expand_dims(np.arange(0, int(max_len)), axis=1)
        index = np.expand_dims(np.arange(0, self.d_model), axis=0)
